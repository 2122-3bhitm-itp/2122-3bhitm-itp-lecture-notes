...
IMPORTANT: Immer bei FROM-Klausel eine Versionsnummer angeben (vergleiche https://semver.org/lang/de/[sematische Versonierung^])

zB `FROM ubuntu:20.04` oder `FROM ubuntu:focal`
...
** Arzt -> Patient, Diagnode, Behandlung, Arzt, Krankenzimmer
...
* im Container (ist nicht wirklich ein persistentes speichern, da die Daten geloescht werden, wenn der Container geloesct wird.)
...
* Im Gegensatz zu virtualbox usw. werden nicht alle Dienste in eine vm installiert, sondern bei Docker wird fuer jeden Dienst ein container erstellt.


* Erstellen eines dockr-compose.yml files
...
* Derzeit kann man auf die DB nur ueber den Adminer zugreifen (ist vergleichbar mit phpadmin)

** http:// localhost:8090

.Nachschauen welcher Dienst lokal laeuft
----
netstat -ant| grep LISTEN
----

.Hinzufuegen eines Ports fuer den direkten Zugriff
[source,yaml]
----
version: '3.1'
...
* Wird die db nun wieder gestartet, so kann man direkt auf die db zugreifen: zB ueber intellij
...
* Beispielsdaten sind zu erstellen (besonders fuer die Stammdaten):
** entweder mittels unit-tests
** oder ueber sql-Statements (in 3 files: create-xx.sql, insert-xx.sql und drop-xx.sql)

* Geschäftsprozesse mit Unit-Tests abbilden zB
